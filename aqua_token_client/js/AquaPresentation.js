...
//initialize profile pic of selected fish
$("#fishProfile").svg({
    settings: {
        height: 100,
        width: 100
...
//initialize pic in mate Modal
$("#mateModalPicture").svg({
    settings: {
        height: 100,
        width: 100
    }
...
aquarium.rect(0, -0.5, 800, 400, {
    fill: 'url(#aquaGrad)',
    stroke: 'black',
    strokeWidth: 10,
    strokeDashArray: "0,800,800"
});
...
setInterval(function() {
    draw()
}, interval, 1); //call draw() each interval
...
function draw() {
    for (var i = 0; i < allFish.length; i++) {
        ...
        if (fish.direction[0] == "l") {
            $(fish.group).animate({
                svgTransform: "translate(" + newX + "," + newY + "), scale(-0.4,0.4)"
            }, interval);
        } else {
            $(fish.group).animate({
                svgTransform: "translate(" + newX + "," + newY + "), scale(0.4)"
            }, interval);
        }
...
function insertToSVG(svgID, fish) {
    ...
    if (svgID == "aquarium") {
        ...
        //add click listener to new fish group
...
//attaches a svg form to given group (to make a fish out of head/tail)
function appendSVGtoGroup(path, svg, group, posX, posY) {
    loadSVG(path).then(function(value) {
        ...
        svg.configure(group, {
            transform: "translate(" + posX + "," + posY + "), scale(0.4)"
        });
    });
}

//loads svg from data source
