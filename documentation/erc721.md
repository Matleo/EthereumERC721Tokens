...
Um einen Mechanismus zu implementieren, in dem die Definition eines `Human` geändert werden kann, muss eine andere, komplexere Vorgehensweise gewählt werden. Das Verwalten der Eigenschaften muss nun in einem anderen Contract geschehen, und nur die Referenz auf die Adresse des Managenden Contracts wird im Hauptcontract festgehalten:
    ...
    function setPropertyManager(address _propertyManager) public {
        propertyManager = _propertyManager;
    }
    
Der PropertyManager Contract erfüllt dann die selbe Funktionalität wie [oben](#contract-mit-eigenschaften) beschrieben, nur das Erstellen läuft anders ab. Das Frontend muss beim Erstellen eines Tokens sowohl den Contract zur Besitzverwaltung, als auch den PropertyManager aufrufen. 

Soll nun eine Eigenschaft dem `Human` hinzugefügt werden, muss:
1. Ein neuer PropertyManager-Contract mit der neuen `struct` und neuen Getter/Setter Methoden deployed werden
2. Die Adresse im Besitzverwaltenden Contract aktualisiert werden
3. Alle Humans mit den Eigenschaften des alten PropertyManagers in den neuen integriert werden und ein Default für das neue Attribut gewählt werden